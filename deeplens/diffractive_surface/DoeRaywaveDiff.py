...
class DoeRaywaveDiff(Aspheric):
    ...
    def refract_wft(self, ray, n1: float, n2: float, n_hat: torch.Tensor, *, seed: int | None = None):
        ...
        new_ray = Ray(o_s, d_world, wvln=wvln_s, is_coherent=(ray.is_coherent or True), device=self.device)
    ...
    def scatter_wft_samples(self, ray, n1: float, n2: float, spr: int | None = None, *, seed: int | None = None):
        ...
        tmp_ray.is_valid = ray.is_valid.clone()
        tmp_ray.is_coherent = getattr(ray, "is_coherent", False)
