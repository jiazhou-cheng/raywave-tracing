...
class FlatDoeSurface:
    ...
    def __post_init__(self) -> None:
        ...
        self.ray_sampling = str(self.ray_sampling).strip().lower()
        if self.ray_sampling not in {"multinomial", "gumbel"}:
            raise ValueError("ray_sampling must be 'multinomial' or 'gumbel'.")
    ...
    def interact(
        ...
        input_field = self.rays_to_field(
            ...
            opl_inc[valid],
            amp=None,
            field_shape=complex_field.shape,
        )
        output_field = nan_to_num_complex(input_field * complex_field)
...
def doe_raywave_plane(
    ...
    *,
    ...
    surface = FlatDoeSurface.plane(
        ...
        pitch_mm=dx_mm,
        wavelength_mm=wavelength_mm,
        aperture_radius_mm=0.5 * int(complex_field.shape[0]) * float(dx_mm),
        origin_xy=origin_xy,
        out_primary_ray_count=out_primary_ray_count,
        sampled_secondary_ray_count=sampled_secondary_ray_count,
        preserve_energy=preserve_energy,
    ...
    return surface.interact(complex_field, o, d)
